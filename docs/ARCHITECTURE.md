...
**Status:** `scripts/verify.sh` and the three smoke suites validate the current
stack. Everything described below was executed and observed, not designed on
paper. Where something does **not** work, or works less well than the architecture
diagram implies, it is said so explicitly - see Section 3.4 and Section 11.
...
MCP container. The claim above is the one this stack actually supports, and Section 3.4
...
The default, `chat`, and `sandbox` services make **17 long-running containers**.
The optional `cubestore` profile adds an 18th; `codeapi-package-init` exists only
...
```mermaid
flowchart TB
  user["Users"] -->|LDAP login| librechat["LibreChat :3080"]
  user -->|LDAP login| superset["Superset :8088"]
  librechat --> ldap["OpenLDAP"]
  superset --> ldap
  librechat -->|LLM inference| foundry["Azure AI Foundry"]
  librechat <--> mongo[("MongoDB")]
  librechat -->|conversation titles and messages| meili[("Meilisearch")]
  librechat -->|user-attached files| rag["RAG API :8000 localhost docs"]
  rag <--> vector[("Postgres: vectordb")]
  librechat --> analystMcp["postgres-mcp analyst"]
  librechat --> adminMcp["postgres-mcp admin"]
  analystMcp -->|static analyst SQL identity| cube["Cube semantic layer"]
  adminMcp -->|static admin SQL identity| cube
  superset -->|shared Cube SQL identity| cube
  cube --> pagila[("Postgres: pagila")]
  librechat --> reviewer["Superset MCP: mcp_reader"] --> superset
  superset <--> supersetDb[("Postgres: superset")]
  librechat --> codeapi["codeapi"] --> sandbox["NsJail sandbox"]
  sandbox --> toolcall["codeapi-toolcall"] --> librechat
  sandbox --> files["codeapi-files"] --> garage[("Garage")]
  codeapi <--> redis[("Redis")]
```

This is the authoritative topology. The compact sketch below focuses on the
low-level data plane and intentionally omits the conversation-search index and
...
| Warehouse tools | `crystaldba/postgres-mcp` x2 | 0.3.0 | pulled, **stock and unmodified** |
...
- **No custom tool server.** An earlier build had one (`cube-mcp`, ~700 lines of
  ...
  testing showed two of its three guards were redundant - see Section 5.1.
...
a replacement Dockerfile to build at all (Section 7, Section 5.5), which is the practical argument
...
LDAP unified the first. It did not touch the second. Section 3.4 says what that costs.
...
Separate views rather than one is a data-driven decision, not tidiness. See Section 6.
...
|---|---|
| Per-request identity via an `X-Cube-User` header | Real, and it is the one thing genuinely lost - see Section 3.4. Though note the header was trusted input from the client anyway |
...
### 5.2 `postgres-mcp` x2 - the masking contrast
...
down the id order and fabricating a trend - are in Section 4. `verify.sh V1b` now
...
- **No 24-month x-axis.** The span is ~7.3 months and a shift relocates a window
  ...
  every rental duration by ~3.2x, taking mean duration from 5.0 to 16.1 days and
...
start when a config file containing secrets is group/world-readable.

### 7.2 Isolation - state this plainly to stakeholders

This Compose profile runs **NsJail-only** rather than the libkrun microVM path.
NsJail **shares the host kernel**. Upstream's own assessment: appropriate for local
...
4. LibreChat        executes the real MCP tool in its own Node process using the
                    agent's statically selected MCP server
...
- **The role follows the agent through PTC.** The tool call reaches the same static
  analyst or admin MCP server selected on that agent record; it does not carry the
...
Cube's pg-wire protocol. Metrics use `MEASURE(x)` for the reason in Section 5.1.
...
anti-pattern. No dual-axis charts: two scales means two charts. No map (Section 6.3). No
sparkline on average rental duration, which has no meaningful trend.

The palette is a validated dark categorical set (worst adjacent CVD DeltaE 8.4 protan,
...
`run-server.sh` defaults to `--workers 1` (gthread x20), which makes in-process
...
Two defects can combine into a silently broken image:

1. Source-tree permissions can leave `/app/packages/*` non-writable. Rolldown
   then cannot create `dist/` and the workspace build fails with `Permission
   denied (os error 13)`.
...
### 9.2 MCP startup and static roles

LibreChat starts three configured MCP servers: `cube_analyst`, `cube_admin`, and
`superset`. The two Cube servers are inspected at startup and expose their tool
lists; the provisioner still constructs stable tool ids as
`<tool>_mcp_<server>` so provisioning does not depend on runtime discovery.

There are no request-context headers in the current MCP configuration. The agent's
tool list selects the analyst or admin server, and that server's static Cube SQL
identity selects the role. This is why the two BI agents demonstrate masking, but
not per-person authorization.
...
no pip install and no container to borrow. It appends the compact shared rules from
`config/librechat/system-rules.md` directly to each system prompt; `file_search`
remains available only for user-attached documents.

`titleConvo` is enabled without a `titleModel` override, so LibreChat uses the
selected agent model for conversation titles. Meilisearch privately indexes those
titles and messages; V24 verifies its authenticated `messages` and `convos`
indexes.
...
| **Authentication** | **OpenLDAP is the single userstore**; both front doors bind to it, group->role mapping works | Fine as a PoC; put TLS on it (Section below) |
...
| Sandbox isolation | **NsJail, shares host kernel** | Use a stronger isolation boundary before executing untrusted code |
...
It does not. See Section 3.4 - and say so out loud when demoing, because the architecture
...
| **V1b** | a breakdown that does not reconcile to its total - the join fan-out that produced 16 plausible bars and a 2.37x over-count |
...
| V8 | required source files missing or unexpectedly empty |
| V9 | non-LF line endings in container-executed source files |
...
| V23 | agent capabilities, no bundled file attachments, compact system rules, RAG URL, and title-model fallback wiring |
| V24 | Meilisearch health, authentication, and `messages`/`convos` index initialization |
...
- A multi-record shell capture retained a record delimiter, so only one of two
  agent records was checked. The current checks read the stored records directly.
...
| **One primary category per film** | keep the many-to-many bridge | additive measures must reconcile to their total; the bridge over-counted revenue by 2.37x. Cost: "which categories does this film have" is no longer answerable |
...
| NsJail | libkrun microVM | current Compose profile uses NsJail; documented as a real limitation |
...
| Date shift, not stretch | fill 24 months | stretching multiplies rental durations 3.2x and makes a KPI lie |
