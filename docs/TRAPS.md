...
- **`public.film_category` is many-to-many here**: 2367 rows for 1000 films, 1-3
  ...
  sliced by category over-reports by ~2.37x - revenue by category came to
...
- Shared BI rules belong in `config/librechat/system-rules.md` and are appended to
  each agent's system prompt during provisioning. Keep `file_search` for documents
  users attach in chat; do not attach bundled rule files to agents.
- Leave `titleModel` unset. `titleConvo: true` then uses the selected agent model
  for conversation titles without introducing a second deployment to maintain.
- Meilisearch is required when `SEARCH=true`: it stays private on the Compose
  network, needs the same `MEILI_MASTER_KEY` as LibreChat, and should contain both
  `messages` and `convos` indexes. `verify.sh V24` checks this.
...
- **Use the `lib.sh` Docker wrappers**: `dexec`, `dcp_to`, `compose_x`, `docker_x`.
  They keep the command environment local when a Docker invocation receives a
  container path.
