#!/usr/bin/env bash
# Bootstrap the single-node Garage cluster. Idempotent, so bootstrap.sh calls it
# unconditionally -- this is what replaces an init container.
#
# Three things are needed before Garage can store anything:
#   1. a LAYOUT, assigned AND applied. Until then the S3 port answers but every
#      write fails, so "the port is open" proves nothing.
#   2. an S3 KEY, IMPORTED from .env rather than generated, so credentials stay
#      deterministic and codeapi-files reads them from the environment.
#   3. a BUCKET plus explicit read/write grant, done here so the key itself needs
#      no create-bucket privilege.
...
case "$ACCESS_KEY" in
  GK*) : ;;
  *) die "MINIO_ACCESS_KEY must be Garage format: 'GK' followed by 24 hex chars (got: $ACCESS_KEY)" ;;
esac
...
# ---- 1. layout --------------------------------------------------------------
# Assigning twice is harmless, but applying an unchanged layout errors -- so act
# only when the node still has no role.
...
if g status 2>/dev/null | grep -qi 'NO ROLE ASSIGNED'; then
  ...
  # Version 1 is the first applied version on a fresh cluster.
