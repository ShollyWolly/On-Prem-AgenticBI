...
| `librechat/` | `vendor/LibreChat` | Upstream swallows a failed frontend build (`npm run frontend;` — note the semicolon), and on Windows that build *always* fails because `COPY` strips the owner write bit. The result builds "successfully" and then crash-loops. |
